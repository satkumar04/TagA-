...
/**
 * Created by SANTECH on 03/29/2018.
 */

public class AccAlarmAdapter extends RecyclerView.Adapter<AccAlarmAdapter.AccAlarmViewHolder> {

    private Context mContext;
    private List<AlarmsBean> accalarmList = new ArrayList<>();
    ...
    public AccAlarmAdapter(Context mContext, List<AlarmsBean> accalarmList, String query) {
        this.mContext = mContext;
        this.accalarmList = accalarmList;
        this.query = query;
    }

    @Override
    public AccAlarmAdapter.AccAlarmViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View itmView = LayoutInflater.from(parent.getContext()).inflate(R.layout.allalarmstxt, parent, false);
        return new AccAlarmAdapter.AccAlarmViewHolder(itmView);
    }

    @Override
    public void onBindViewHolder(AccAlarmAdapter.AccAlarmViewHolder holder, int position) {
        AlarmsBean vhclbn = accalarmList.get(position);
        ...
        if(vhclbn.getAlarmtype().equals("AccOff")) {
            holder.txt_door.setText("IGN OFF");
        }
        if(vhclbn.getAlarmtype().equals("AccOn")) {
            holder.txt_door.setText("IGN ON");
        }
    ...
    public final class AccAlarmViewHolder extends RecyclerView.ViewHolder implements View.OnClickListener {

        TextView txt_date, txt_vehiclename,txt_address,txt_door;
        LinearLayout lv_allalarms;
        // ImageView imgvehiclests;

        public AccAlarmViewHolder(View itemView) {
            super(itemView);
            ...
            txt_door = (TextView) itemView.findViewById(R.id.txtdoor);
