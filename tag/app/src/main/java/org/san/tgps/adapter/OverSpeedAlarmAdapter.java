...
public class OverSpeedAlarmAdapter extends RecyclerView.Adapter<OverSpeedAlarmAdapter.OverSpeedAlarmViewHolder> {

    private Context mContext;
    private List<AlarmsBean> overspeedalarmList = new ArrayList<>();
    private String query;
    private Spannable spannable;
    private ColorStateList colorstate;
    private TextAppearanceSpan highlightSpan;

    public OverSpeedAlarmAdapter(Context mContext, List<AlarmsBean> overspeedalarmList, String query) {
        this.mContext = mContext;
        this.overspeedalarmList = overspeedalarmList;
        this.query = query;
    }

    @Override
    public OverSpeedAlarmAdapter.OverSpeedAlarmViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View itmView = LayoutInflater.from(parent.getContext()).inflate(R.layout.allalarmstxt, parent, false);
        return new OverSpeedAlarmAdapter.OverSpeedAlarmViewHolder(itmView);
    }

    @Override
    public void onBindViewHolder(OverSpeedAlarmAdapter.OverSpeedAlarmViewHolder holder, int position) {
        AlarmsBean vhclbn = overspeedalarmList.get(position);
    ...
    public int getItemCount() {
        return overspeedalarmList.size();
    }
    ...
    public final class OverSpeedAlarmViewHolder extends RecyclerView.ViewHolder implements View.OnClickListener {

        TextView txt_date, txt_vehiclename,txt_address;
        LinearLayout lv_allalarms;
        // ImageView imgvehiclests;

        public OverSpeedAlarmViewHolder(View itemView) {
            super(itemView);
