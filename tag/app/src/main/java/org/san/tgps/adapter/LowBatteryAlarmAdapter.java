...
public class LowBatteryAlarmAdapter extends RecyclerView.Adapter<LowBatteryAlarmAdapter.LowBatteryAlarmViewHolder> {

    private Context mContext;
    private List<AlarmsBean> lowbatteryalarmList = new ArrayList<>();
    private String query;
    private Spannable spannable;
    private ColorStateList colorstate;
    private TextAppearanceSpan highlightSpan;

    public LowBatteryAlarmAdapter(Context mContext, List<AlarmsBean> lowbatteryalarmList, String query) {
        this.mContext = mContext;
        this.lowbatteryalarmList = lowbatteryalarmList;
        this.query = query;
    }

    @Override
    public LowBatteryAlarmAdapter.LowBatteryAlarmViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View itmView = LayoutInflater.from(parent.getContext()).inflate(R.layout.allalarmstxt, parent, false);
        return new LowBatteryAlarmAdapter.LowBatteryAlarmViewHolder(itmView);
    }

    @Override
    public void onBindViewHolder(LowBatteryAlarmAdapter.LowBatteryAlarmViewHolder holder, int position) {
        AlarmsBean vhclbn = lowbatteryalarmList.get(position);
    ...
    public int getItemCount() {
        return lowbatteryalarmList.size();
    }
    ...
    public final class LowBatteryAlarmViewHolder extends RecyclerView.ViewHolder implements View.OnClickListener {

        TextView txt_date, txt_vehiclename,txt_address;
        LinearLayout lv_allalarms;
        // ImageView imgvehiclests;

        public LowBatteryAlarmViewHolder(View itemView) {
            super(itemView);
