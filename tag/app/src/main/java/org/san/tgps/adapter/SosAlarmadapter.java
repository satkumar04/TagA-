...
public class SosAlarmadapter extends RecyclerView.Adapter<SosAlarmadapter.SosAlarmViewHolder> {

    private Context mContext;
    private List<AlarmsBean> sosalarmList = new ArrayList<>();
    private String query;
    private Spannable spannable;
    private ColorStateList colorstate;
    private TextAppearanceSpan highlightSpan;

    public SosAlarmadapter(Context mContext, List<AlarmsBean> sosalarmList, String query) {
        this.mContext = mContext;
        this.sosalarmList = sosalarmList;
        this.query = query;
    }

    @Override
    public SosAlarmadapter.SosAlarmViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View itmView = LayoutInflater.from(parent.getContext()).inflate(R.layout.allalarmstxt, parent, false);
        return new SosAlarmadapter.SosAlarmViewHolder(itmView);
    }

    @Override
    public void onBindViewHolder(SosAlarmadapter.SosAlarmViewHolder holder, int position) {
        AlarmsBean vhclbn = sosalarmList.get(position);
    ...
    public final class SosAlarmViewHolder extends RecyclerView.ViewHolder implements View.OnClickListener {

        TextView txt_date, txt_vehiclename,txt_address;
        LinearLayout lv_allalarms;
       // ImageView imgvehiclests;

        public SosAlarmViewHolder(View itemView) {
            super(itemView);
